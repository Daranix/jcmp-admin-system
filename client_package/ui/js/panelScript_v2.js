function loadVue() {
    ...
    var adminSys = {
        footerOptions: [
            { name: "Ban", minrank: 1, action: 'banPlayer', params: [] },
            { name: "Kick", minrank: 1, action: 'kickPlayer', params: [] },
            { name: "Teleport", minrank: 1, action: 'tpPlayer', params: [] },
            //{ name: "Freeze", minrank: 1, action: 'freezePlayer'},
            { name: "Set HP", minrank: 1, action: 'setHP', params: [] },
            { name: "Set admin rank", minrank: 2, action: 'setAdminRank', params: []},
            { name: "Give Weapon", minrank: 1, action: 'weaponMenu', params: [] },
            { name: "Toggle wingsuit", minrank: 1, action: 'toggleAbility', params: ['wingsuit'] },
            { name: "Toggle parachute", minrank: 1, action: 'toggleAbility', params: ['parachute'] },
            { name: "Toggle grappel", minrank: 1, action: 'toggleAbility', params: ['grappling_hook'] },
            { name: "Godmode", minrank: 1, action: 'toggleGodmode', params: [] }
        ],
        localPlayer: {}
    }
    ...
    var optionBoxCtrl = new Vue({
        ...
        methods: {
            ...
            executeAction: function(event, index) {
                ...
                var params = adminSys.footerOptions[option].params;

                Action[action](player, ...params);
            },
        }
    });
    ...
    var spawnMenuCtrl = new Vue({
        ...
        methods: {
            spawnWeapon: function(weaponhash) {
                Action.spawnWeapon(playerInfoCtrl.selectedPlayer, weaponhash, this.ammoQuantity);
            }
        }
    });
